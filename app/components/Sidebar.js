    ...
    <>
      <header className="topnav">
        <div className="topnav-inner">
          ...
          <nav className="topnav-links">
            {NAV_ITEMS.map((item) => (
              <Link key={item.href} href={item.href} className={`topnav-link ${isActive(item.href) ? 'active' : ''}`}>
                {item.label}
              </Link>
            ))}
      ...
      <nav className="mobile-nav">
        {MOBILE_NAV.map((item) => {
          if (item.href === '/more') {
            return (
              <button key="more" className={`mobile-nav-item ${mobileMoreOpen ? 'active' : ''}`} onClick={() => setMobileMoreOpen(!mobileMoreOpen)}>
          ...
          return (
            <Link key={item.href} href={item.href} className={`mobile-nav-item ${isActive(item.href) ? 'active' : ''}`} onClick={() => setMobileMoreOpen(false)}>
        ...
        <div className="mobile-more-overlay" onClick={() => setMobileMoreOpen(false)}>
          <div className="mobile-more-sheet" onClick={(e) => e.stopPropagation()}>
            {[...NAV_ITEMS.slice(3), ...MORE_ITEMS].map((item) => (
              <Link key={item.href} href={item.href} className="mobile-more-item" onClick={() => setMobileMoreOpen(false)}>{item.label}</Link>
            ))}
          </div>
        </div>
      )}
      ...
      <style jsx>{`
        ...
        .topnav-brand { text-decoration: none; flex-shrink: 0; }
        .brand-text {
        ...
        .topnav-more-btn { cursor: pointer; }
        .topnav-dropdown {
          display: none; position: absolute;
          top: calc(100% + 12px); right: -20px;
          background: var(--surface); border: 1px solid var(--border);
          border-radius: 12px; box-shadow: var(--shadow-lg);
          min-width: 180px; padding: 8px; z-index: 200;
        }
        .topnav-more:hover .topnav-dropdown { display: block; }
        .topnav-dropdown-item {
          display: block; padding: 10px 16px;
          font-size: 13px; font-weight: 400; color: var(--text-soft);
          border-radius: 8px; transition: all 0.12s; text-decoration: none;
        }
        .topnav-dropdown-item:hover { background: var(--bg); color: var(--text); }
        .topnav-actions { display: flex; align-items: center; gap: 12px; flex-shrink: 0; }
        .topnav-icon-btn {
        ...
        .topnav-icon-btn:hover { background: var(--accent-glow); color: var(--text); }
        .topnav-avatar {
        ...
        .mobile-nav {
          ...
          height: var(--mobile-nav-height); background: var(--surface);
        ...
        .mobile-nav-item {
          ...
          transition: color 0.15s ease; min-width: 56px;
        ...
        .mobile-more-sheet {
          position: absolute; bottom: var(--mobile-nav-height);
          left: 12px; right: 12px; background: var(--surface);
          border-radius: 16px 16px 0 0; padding: 16px; box-shadow: var(--shadow-lg);
        }
        .mobile-more-item {
